...
// Rota: /s10-raio-x/[place_id] → JSX render com morph + corpus
// SWC-ready: inline props, funções auxiliares fora, zero catch vazio.
...
async function fetchOutput(placeId: string) {
  try {
    const result = await composeS10(placeId)
    if (!result) return null
    const meta = result.meta as Record<string, any>
    // Build S10BlueOutput from real pipeline data
    return {
      ...
      tracedLayout: meta.layoutTree || { id: 'layout.s10', type: 's10-raio-x', slots: {
        hero: {}, score: {}, info_grid: {}, gaps: {}, cta: {}, footer: {}
      }},
...
export default async function S10RaioXRoute(props: { params: Promise<{ place_id: string }> }) {
  ...
  const output = await fetchOutput(place_id)
